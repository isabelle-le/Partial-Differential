function[]=Solution_BS_PutOption()
K=10;
L=2*K;
T=1;
M=4999; 
...
hold on;
for n=1:M+1
    V(n,N+2)= Boundary_Condition1(t(n));
    V(n,1)=Boundary_Condition_Put2(t(n),r,T,K);
end
figure;
plot(t,V(:,1),'*');
hold on;
...
for n=M+2:-1:2
    for i=2:N+1
        V(n-1,i)= V(n,i)+ delt*r*S(i)*(V(n,i+1)-V(n,i-1))/(2*dels)+...
            delt*sigma^2 * S(i)^2 *(V(n,i+1)+V(n,i-1)-2*V(n,i))/...
            (2*dels^2)- r*delt* V(n,i);
    end
end
figure;
plot(S,V(1,:));
